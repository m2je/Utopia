<%@page import="ir.utopia.nejat2.inventory.takingofinventoryperiod.bean.InBdTakingInvntoryPriodFacadeRemote"%>
...
<title><%=MessageHandler.getMessage("InventoryTagTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('takingInvntoryPriod').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("takingInvntoryPriod",path, lang))%>');
		return false;
	}
	return true;
...
function report(reportType){
	if(validate()){
		...
		var takingInvntoryPriodId=document.getElementById('takingInvntoryPriod').value;
		var url = "<%=UsecaseUtil.getActionUrl("In_Rp_InventoryTag",predefindedActions.report.toString())%>"+"?takingInvntoryPriodId="+takingInvntoryPriodId+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="tag_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;" width="90">
           <%=MessageHandler.getMessage("takingInvntoryPriod",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel" width="208">
           <select name="takingInvntoryPriod" id="takingInvntoryPriod" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       	InBdTakingInvntoryPriodFacadeRemote tIPFacade=(InBdTakingInvntoryPriodFacadeRemote)ServiceFactory.lookupFacade(InBdTakingInvntoryPriodFacadeRemote.class);
	       	       List<NamePair>pairs= tIPFacade.loadLookup(context, null);
