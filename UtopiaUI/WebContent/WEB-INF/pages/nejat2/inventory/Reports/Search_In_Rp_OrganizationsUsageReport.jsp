...
<title><%=MessageHandler.getMessage("OrganizationsUsageTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var url = "<%=UsecaseUtil.getActionUrl("In_Rp_OrganizationsUsageReport",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&inventoryId="+inventoryId+"&merchanId="+merchanId+"&userId="+userId+"&orgId="+orgId+"&fiscalyearId="+fiscalyearId+"&reportType="+reportType;
		if(url.length == 0){
