...
<title><%=MessageHandler.getMessage("KardksTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var merchandise=document.getElementById('merchandise_C').value;
		var url = "<%=UsecaseUtil.getActionUrl("In_Rp_MerchandiseKardks",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&inventory="+inventory+"&merchandise="+merchandise+"&reportType="+reportType;
		if(url.length == 0){
...
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       ...
       <td align="right" class="clsLabel">
           <select name="inventory" id="inventory" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
       ...
       <td align="right" class="clsLabel">
                                 <tag:LOV usecaseName="In_Bd_Merchandise" formClassName="<%=ir.utopia.nejat2.inventory.merchandise.action.MerchandiseForm.class%>" widgetName="merchandise" searchTitle="merchandiseSearch" displayItems="name"></tag:LOV>
	   </td>
       <td>&nbsp;
       </td>
     </tr>
