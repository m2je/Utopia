...
<title><%=MessageHandler.getMessage("MerchanRequestTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var url = "<%=UsecaseUtil.getActionUrl("In_Rp_MerchanRequestReport",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&merchanId="+merchanId+"&userId="+userId+"&orgId="+orgId+"&fiscalyearId="+fiscalyearId+"&reportType="+reportType;
		if(url.length == 0){
