...
function report(reportType){
	if(validate()){
		...
		var merchanGroupId=document.getElementById('merchanGroupId').value;
		var userId=document.getElementById('userId').value;
		var fiscalyearId=document.getElementById('fiscalyearId').value;
		var url = "<%=UsecaseUtil.getActionUrl("In_Rp_MerchanRecptReport",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&inventoryId="+inventoryId+"&merchanGroupId="+merchanGroupId+"&userId="+userId+"&fiscalyearId="+fiscalyearId+"&reportType="+reportType;
		if(url.length == 0){
...
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       ...
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("merchandiseGroup",path, lang)%>
       </td>
       <td align="right" class="clsLabel">
           <select name="merchanGroupId" id="merchanGroupId" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       	InBdMerchandiseGroupFacadeRemote merchanFacade=(InBdMerchandiseGroupFacadeRemote)ServiceFactory.lookupFacade(InBdMerchandiseGroupFacadeRemote.class);
	       	       List<NamePair>merchanPairs= merchanFacade.loadLookup(context, null);
	       	       for(NamePair merchanPair:merchanPairs){
	       %>
	       <option value=<%= merchanPair.getKey()%>><%=WebUtil.write(merchanPair,lang) %></option>
	       <%} %>
	       </select>
	   </td>
       <td>&nbsp;
