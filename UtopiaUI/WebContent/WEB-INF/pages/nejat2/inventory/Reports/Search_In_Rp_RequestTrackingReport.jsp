...
<title><%=MessageHandler.getMessage("RequestTrackingTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('orgId').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("orgId",path, lang))%>');
		return false;
	}
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("In_Rp_RequestTrackingReport",predefindedActions.report.toString())%>"+"?orgId="+orgId+"&fiscalYear="+fiscalYear+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="RequestTrackingReport_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;" width="90";">
           <%=MessageHandler.getMessage("orgId",path, lang)%>
     ...
     <tr>   
       ...
       <td align="right" class="clsLabel">
	       ...
	       CmFiscalyearFacadeRemote FiscalYearFacade=(CmFiscalyearFacadeRemote)ServiceFactory.lookupFacade(CmFiscalyearFacadeRemote.class);
   	       List<NamePair>fypairs= FiscalYearFacade.loadLookup(context, null);
   	       for(NamePair fyPair:fypairs){ 
           %>
            <option value=<%=fyPair.getKey()%>><%=WebUtil.write(fyPair,lang)%></option>
           <%
   	       }
           %>
	       </select>
