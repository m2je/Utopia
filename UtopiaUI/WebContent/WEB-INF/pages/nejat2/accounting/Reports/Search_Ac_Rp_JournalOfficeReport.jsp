...
<title><%=MessageHandler.getMessage("JournalOfficeTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var url = "<%=UsecaseUtil.getActionUrl("Ac_Rp_JournalOfficeReport",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&documentFrom="+documentFrom+"&documentTo="+documentTo+"&OrganizationId="+OrganizationId+"&FiscalYear="+FiscalYear+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="JournalOfficeRepor_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
    ...
    <%--  <tr>
       ...
       <td align="right" class="clsLabel">
           <select name="documentFrom" id="documentFrom" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       AcDocumentsFacadeRemote fDocumentFacade=(AcDocumentsFacadeRemote)ServiceFactory.lookupFacade(AcDocumentsFacadeRemote.class);
	       	       List<NamePair>fDocumentPairs= fDocumentFacade.loadLookup(context, null);
	       	       for(NamePair fDocumentPair:fDocumentPairs){ 
	       %>
	       <option value=<%=fDocumentPair.getKey()%>><%=WebUtil.write(fDocumentPair,lang)%></option>
	       <%
	       	}
	       %>
	       </select>
     ...
     <td align="right" class="clsLabel">
           <select name="documentTo" id="documentTo" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       AcDocumentsFacadeRemote tDocumentsFacade=(AcDocumentsFacadeRemote)ServiceFactory.lookupFacade(AcDocumentsFacadeRemote.class);
	       	       List<NamePair>tDocumentsPairs= tDocumentsFacade.loadLookup(context, null);
	       	       for(NamePair tDocumentsPair:tDocumentsPairs){ 
	       %>
	       <option value=<%=tDocumentsPair.getKey()%>><%=WebUtil.write(tDocumentsPair,lang)%></option>
	       <%
	       	}
	       %>
	       </select>
