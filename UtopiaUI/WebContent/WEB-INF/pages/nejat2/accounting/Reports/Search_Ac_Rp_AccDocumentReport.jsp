...
<title><%=MessageHandler.getMessage("AccDocumentReportTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var url = "<%=UsecaseUtil.getActionUrl("Ac_Rp_AccDocumentReportReport",predefindedActions.report.toString())%>"+"?dateFrom="+dateFrom+"&dateTo="+dateTo+"&documentFrom="+documentFrom+"&documentTo="+documentTo+"&OrganizationId="+OrganizationId+"&FiscalYear="+FiscalYear+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="AccDocumentReportRepor_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
