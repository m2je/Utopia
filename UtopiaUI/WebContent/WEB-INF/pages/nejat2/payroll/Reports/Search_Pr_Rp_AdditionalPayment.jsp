...
<title><%=MessageHandler.getMessage("AdditionalPaymentTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_AdditionalPayment",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&PersonnelId="+selected+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="AdditionalPayment_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
