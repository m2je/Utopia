...
<title><%=MessageHandler.getMessage("PersonnelListTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
		...
		var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_PersonnelList",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&reportType="+reportType;
		if(url.length == 0){
...
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       ...
       <td align="right" class="clsLabel" width="208">
           <select name="OrganizationId" id="OrganizationId" class="clsSelect">
           <option value=""></option>
	       <%
	       PrVOrganizationFacadeRemote PersonnelListFacade=(PrVOrganizationFacadeRemote)ServiceFactory.lookupFacade(PrVOrganizationFacadeRemote.class);
