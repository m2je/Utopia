...
<title><%=MessageHandler.getMessage("TotalSalaryListTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_TotalSalaryList",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="TotalSalaryList_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
