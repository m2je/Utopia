...
<title><%=MessageHandler.getMessage("PaymentRequestOvertimeTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('OrganizationId').value==""){
		...
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("HiringType",path, lang))%>');
		return false;
	}
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_PaymentRequestOvertime",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="PaymentRequestOvertime_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
