...
<title><%=MessageHandler.getMessage("TaxControlTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_TaxControl",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&PersonnelId="+selected+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="TaxControl_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
