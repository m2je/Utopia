...
<title><%=MessageHandler.getMessage("MsioControlTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_MsioControl",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&PersonnelId="+selected+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="MsioControl_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
