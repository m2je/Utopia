...
<title><%=MessageHandler.getMessage("PayStubsTitle",path, lang)%></title>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_PayStubs",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&PersonnelId="+selected+"&kind="+kind+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="PayStubs_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
