...
<%@page import="ir.utopia.nejat2.payroll.hiringtype.bean.PrBdHiringTypeFacadeRemote" %>
<%@page import="ir.utopia.nejat2.payroll.taminworkshops.bean.PrBdTaminWorkshopsFacadeRemote" %>
...
function report(reportType){
	if(validate()){
		...
		var HiringType=document.getElementById('HiringType').value;
		var MonthCalc=document.getElementById('MonthCalc').value;
		var workshop =document.getElementById('workshop').value;
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_TaminReport2",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&HiringType="+HiringType+"&MonthCalc="+MonthCalc+"&workshop="+workshop+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="TaminReport2_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       ...
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("workshop",path, lang)%>
       </td>
       <td align="right" class="clsLabel">
           <select name="workshop" id="workshop" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       PrBdTaminWorkshopsFacadeRemote wFacade=(PrBdTaminWorkshopsFacadeRemote)ServiceFactory.lookupFacade(PrBdTaminWorkshopsFacadeRemote.class);
	       	       List<NamePair>wPairs= wFacade.loadLookup(context, null);
	       	       for(NamePair wPair:wPairs){
	       %>
	       <option value=<%= wPair.getKey()%>><%=WebUtil.write(wPair,lang) %></option>
	       <%} %>
	       </select>
	              </td>
       <td>&nbsp;    
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("HiringType",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel">
           <select name="HiringType" id="HiringType" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       PrBdHiringTypeFacadeRemote HiringTypeFacade=(PrBdHiringTypeFacadeRemote)ServiceFactory.lookupFacade(PrBdHiringTypeFacadeRemote.class);
	       	       List<NamePair>htpairs= HiringTypeFacade.loadLookup(context, null);
	       	       for(NamePair htPair:htpairs){ 
	       %>
	       <option value=<%=htPair.getKey()%>><%=WebUtil.write(htPair,lang)%></option>
	       <%
	       	}
	       %>
	       </select>
       ...
       <td align="right" class="clsLabel">
           ...
           <option value=""></option>
	       <%
	       	PrPrMonthCalcFacadeRemote mcFacade=(PrPrMonthCalcFacadeRemote)ServiceFactory.lookupFacade(PrPrMonthCalcFacadeRemote.class);
	       	       List<NamePair>mcPairs= mcFacade.loadLookup(context, null);
    ...
    <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("kind",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel">
           <select name="kind" id="kind" class="clsSelect" style="width:200px;">
           <option value=""></option>
	                   <%--  <option value="0"><%=MessageHandler.getMessage("current & outstanding",path, lang)%></option> --%>
						<option value="1"><%=MessageHandler.getMessage("current",path, lang)%></option>
						<option value="2"><%=MessageHandler.getMessage("outstanding",path, lang)%></option>
	     </select>
       </td>
       <td width="90">&nbsp;    
       </td>
