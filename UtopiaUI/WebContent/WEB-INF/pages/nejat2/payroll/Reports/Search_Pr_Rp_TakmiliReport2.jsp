...
<%@page import="ir.utopia.nejat2.payroll.insurancetype.bean.PrBdInsuranceTypeFacadeRemote" %>
...
<title><%=MessageHandler.getMessage("TakmiliReport2Title",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('OrganizationId').value==""){
		...
		if(document.getElementById('InsuranceType').value==""){
			alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
					lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("InsuranceType",path, lang))%>');
			return false;
		}
	return true;
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_TakmiliReport2",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&InsuranceType="+InsuranceType+"&MonthCalc="+MonthCalc+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="TakmiliReport2_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("InsuranceType",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel">
           <select name="InsuranceType" id="InsuranceType" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       PrBdInsuranceTypeFacadeRemote InsuranceTypeFacade=(PrBdInsuranceTypeFacadeRemote)ServiceFactory.lookupFacade(PrBdInsuranceTypeFacadeRemote.class);
	       	       List<NamePair>itpairs= InsuranceTypeFacade.loadLookup(context, null);
	       	       for(NamePair itPair:itpairs){ 
	       %>
	       <option value=<%=itPair.getKey()%>><%=WebUtil.write(itPair,lang)%></option>
	       <%
	       	}
	       %>
	       </select>
