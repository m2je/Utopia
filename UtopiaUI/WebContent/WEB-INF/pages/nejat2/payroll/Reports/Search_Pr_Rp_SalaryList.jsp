...
<title><%=MessageHandler.getMessage("SalaryListTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('OrganizationId').value==""){
		...
		if(document.getElementById('kind').value==""){
			alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
					lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("kind",path, lang))%>');
			return false;
		}
		return true;
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_SalaryList",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&PersonnelId="+selected+"&kind="+kind+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="SalaryList_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
