...
<%@page import="ir.utopia.nejat2.payroll.deductionsdefinition.bean.PrBdDeductionsDefinitionFacadeRemote"%>
...
<title><%=MessageHandler.getMessage("DeductionListTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('OrganizationId').value==""){
		...
		<%--  var personnel=document.getElementById('deductionId_C');
		 if(personnel.options.length==0){
			alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
					lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("PersonnelId",path, lang))%>');
			return false;
		} 
		 --%>
		return true;
...
function report(reportType){
	if(validate()){
		...
		var personnel=document.getElementById('deductionsdefinitionId_C');
		selected = new Array(); 
		for (var i = 0; i < personnel.options.length; i++) 
				selected.push(personnel.options[i].value);
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_DeductionList",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&MonthCalc="+MonthCalc+"&DeductionId="+selected+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="DeductionList_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;" width="90">
           <%=MessageHandler.getMessage("DeductionId",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel" width="208">
           
          <tag:LOV usecaseName="Pr_Bd_DeductionsDefinition" formClassName="<%=ir.utopia.nejat2.payroll.deductionsdefinition.action.DeductionsDefinitionForm.class%>" widgetName="deductionsdefinitionId" searchTitle="deductionSearch" displayItems="code,name" separator=" - " multiSelect="true"></tag:LOV>
   
 <%-- <tag:LOV usecaseName="Pr_Bd_Personnel" formClassName="<%=ir.utopia.nejat2.payroll.personnel.action.PersonnelForm.class%>" widgetName="personnelId" searchTitle="personnelSearch" displayItems="name,secound_name,code" separator=" " multiSelect="true"></tag:LOV> --%>
       </td>
       <td width="90">&nbsp;    
       </td>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;" width="90">
