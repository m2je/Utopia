...
<%@page import="ir.utopia.common.basicinformation.branch.bean.CmBranchFacadeRemote" %>
...
function validate(){
	...
	<%-- 	if(document.getElementById('BranchId').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("BranchId",path, lang))%>');
		return false;
	} --%>
...
function report(reportType){
	if(validate()){
	    ...
	    var url = "<%=UsecaseUtil.getActionUrl("Pr_Rp_BankReport",predefindedActions.report.toString())%>"+"?OrganizationId="+OrganizationId+"&BranchId="+BranchId+"&PersonnelId="+selected+"&reportType="+reportType;
		if(url.length == 0){
...
<form name="BankReport_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
<TABLE  CELLSPACING="1" CELLPADDING="1" width="800" align="center">
     ...
     <tr>
       <td align="right" class="clsLabel" style="font-family: Tahoma;font-size:12px;">
           <%=MessageHandler.getMessage("BranchId",path, lang)%>
           
       </td>
       <td align="right" class="clsLabel">
           <select name="BranchId" id="BranchId" class="clsSelect" style="width:200px;">
           <option value=""></option>
	       <%
	       CmBranchFacadeRemote brFacade=(CmBranchFacadeRemote)ServiceFactory.lookupFacade(CmBranchFacadeRemote.class);
	       	       List<NamePair>htpairs= brFacade.loadLookup(context, null);
