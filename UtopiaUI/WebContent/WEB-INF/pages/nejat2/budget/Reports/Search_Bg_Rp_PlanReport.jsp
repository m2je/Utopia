...
<title><%=MessageHandler.getMessage("PlanReportTitle",path, lang)%></title>
...
function validate(){
	...
	if(document.getElementById('PlanType').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("PlanType",path, lang))%>');
		return false;
	}
	return true;
...
function report(reportType){
	 if(validate())
	{
		...
		var url = "<%=UsecaseUtil.getActionUrl("Bg_Rp_PlanReport",predefindedActions.report.toString())%>"+"?PlanType="+PlanType+"&reportType="+reportType;
		if(url.length == 0){
...
<body class="clsInputPage">
	<form name="PlanReport_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
		<table cellspacing="1" cellpadding="1" width="800" align="center">
			...
			<tr>
				<td align="right" class="clsLabel" style="font-family: Tahoma; font-size: 12px;" width="90"><%=MessageHandler.getMessage("PlanType",path, lang)%>
				</td>
				<td align="right" class="clsLabel" width="208"><select name="PlanType" id="PlanType" class="clsSelect">
						<option value=""></option>
						<%--  <%
	       BgBdPlanFacadeRemote planFacade=(BgBdPlanFacadeRemote)ServiceFactory.lookupFacade(BgBdPlanFacadeRemote.class);
	       	       List<NamePair>pairs=planFacade.loadLookup(context, null);
