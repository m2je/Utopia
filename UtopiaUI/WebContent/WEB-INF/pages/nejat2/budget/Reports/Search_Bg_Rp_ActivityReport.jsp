...
<%@page import="ir.utopia.nejat2.budget.activity.bean.BgBdActivityFacadeRemote"%>
...
<title><%=MessageHandler.getMessage("ActivityReportTitle",path, lang)%></title>
...
<%-- function validate(){
	...
	if(document.getElementById('activityType').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("activityType",path, lang))%>');
		return false;
	}
	return true;
...
function report(reportType)
{
	 ...
	 
	{
		...
		var url = "<%=UsecaseUtil.getActionUrl("Bg_Rp_ActivityReport",predefindedActions.report.toString())%>"+"?activityType="+activityType+"&reportType="+reportType;
		if(url.length == 0){
...
<body class="clsInputPage">
	<form name="ActivityReport_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
		<table cellspacing="1" cellpadding="1" width="800" align="center">
			...
			<tr>
				...
				<td align="right" class="clsLabel" width="208"><select name="activityType" id="activityType" class="clsSelect">
						<option value=""></option>
						<%--   <%
	       BgBdActivityFacadeRemote activityFacade=(BgBdActivityFacadeRemote)ServiceFactory.lookupFacade(BgBdActivityFacadeRemote.class);
	       	       List<NamePair>pairs=activityFacade.loadLookup(context, null);
