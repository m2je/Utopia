...
<title><%=MessageHandler.getMessage("ProjectPlanReportTitle",path, lang)%></title>
...
<%-- function validate(){
	...
	if(document.getElementById('PlanId').value==""){
		alert('<%=WebUtil.write(MessageHandler.getMessage("inValidDate","ir.utopia.core.util.tags.datainputform.client.DataInputFormConstants",
				lang),lang).replace("\"@dateItem@\"",MessageHandler.getMessage("PlanId",path, lang))%>');
		return false;
	}
	return true;
...
function report(reportType){
	/* if(validate()){ */
		...
		var url = "<%=UsecaseUtil.getActionUrl("Bg_Rp_ProjectsPlanReport",predefindedActions.report.toString())%>"+"?PlanId="+PlanId+"&reportType="+reportType;
		if(url.length == 0){
...
<body class="clsInputPage">
	<form name="ProjectsPlan_search" method="POST" target="_self" dir="rtl" action="--WEBBOT-SELF--">
		<table cellspacing="1" cellpadding="1" width="800" align="center">
			...
			<tr>
				...
				<td align="right" class="clsLabel" width="208"><select name="PlanId" id="PlanId" class="clsSelect">
						<option value=""></option>
						<%
