...
<%@page import="ir.utopia.nejat2.budget.department.bean.BgBdDepartmentFacadeRemote"%>
...
function report(reportType){
	if(validate()){
        ...
        var url = "<%=UsecaseUtil.getActionUrl("Bg_Rp_DepartmentPlanReport",predefindedActions.report.toString())%>"+"?fiscalyearId="+fiscalyearId+"&departmentId="+departmentId+"&reportType="+reportType;
		if(url.length == 0){
		...
		<table cellspacing="1" cellpadding="1" width="800" align="center">
			...
			<tr>
				...
				>
						<option value=""></option>
						<%
	       CmFiscalyearFacadeRemote fyFacade=(CmFiscalyearFacadeRemote)ServiceFactory.lookupFacade(CmFiscalyearFacadeRemote.class);
	       	       List<NamePair>pairs= fyFacade.loadLookup(context, null);
	       ...
	       %>
			...
			<tr>
				<td align="right" class="clsLabel" style="font-family: Tahoma; font-size: 12px;"><%=MessageHandler.getMessage("departmentId",path, lang)%>
				</td>
				<td align="right" class="clsLabel"><select name="departmentId" id="departmentId" class="clsSelect"
					style="width: 200px;"
				>
						<option value=""></option>
						<%
	       BgBdDepartmentFacadeRemote depFacade=(BgBdDepartmentFacadeRemote)ServiceFactory.lookupFacade(BgBdDepartmentFacadeRemote.class);
	       	       List<NamePair>dppairs= depFacade.loadLookup(context, null);
	       	       for(NamePair dpPair:dppairs){ 
	       %>
						<option value="<%=dpPair.getKey()%>"><%=WebUtil.write(dpPair,lang)%></option>
						<%
	       	}
	       %>
				</select></td>
